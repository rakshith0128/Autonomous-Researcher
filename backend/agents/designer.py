...
class ExperimentDesigner(BaseAgent):
    ...
    def _validate(spec: ExperimentSpec, dataset: Dataset) -> str | None:
        ...
        # Identifier-like columns are not categories. A live run proposed
        # classifying on `source_url`, where every row holds a unique value --
        # 60 classes with one example each. That is not a classification
        # problem, it is a lookup table, and cross-validation on it is
        # meaningless rather than merely weak.
        for key in ("target", "group"):
            name = spec.params.get(key)
            if not isinstance(name, str) or name not in dataset.data:
                continue
            values = [v for v in dataset.data[name] if v is not None]
            distinct = len({str(v) for v in values})
            if values and distinct > max(2, len(values) // 3):
                return (
                    f"{key}={name!r} has {distinct} distinct values across {len(values)} "
                    "rows, so it identifies rows rather than grouping them. Choose a "
                    "column with a small number of repeated categories."
                )
