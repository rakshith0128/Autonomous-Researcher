...
class VectorMemory:
    ...
    def initialise(self) -> bool:
        ...
        Returns False if unavailable or switched off. Retrieval is an
        enhancement: losing it should degrade the depth of evidence the agents
        see, never end the run.
        """
        from ..config import get_settings

        if not get_settings().enable_vector_memory:
            log.info("vector memory disabled by configuration")
            self._available = False
            return False
